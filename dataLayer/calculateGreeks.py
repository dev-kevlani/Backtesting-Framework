...
def calculate_iv_and_greeks(options_data):
    ...
    try:
        ...
        options_data['time_to_expiry'] = options_data['time_diff'].dt.total_seconds() / (24 * 3600)  # Calculate total seconds for better precision
    ...
    try:
        ...
        delta = vectorized_delta(option_types, spot_prices, strike_prices, times_to_expiry, risk_free_rates, iv,
                                    q=0, model='black_scholes', return_as='numpy')
        gamma = vectorized_gamma(option_types, spot_prices, strike_prices, times_to_expiry, risk_free_rates, iv,
                                    q=0, model='black_scholes', return_as='numpy')
        rho = vectorized_rho(option_types, spot_prices, strike_prices, times_to_expiry, risk_free_rates, iv,
                                q=0, model='black_scholes', return_as='numpy')
        theta = vectorized_theta(option_types, spot_prices, strike_prices, times_to_expiry, risk_free_rates, iv,
                                    q=0, model='black_scholes', return_as='numpy')
    except ZeroDivisionError as e:
        print(f"ZeroDivisionError Error occurred: {e}")
        iv = np.full(len(options_data), np.nan)
        delta = np.full(len(options_data), np.nan)
        gamma = np.full(len(options_data), np.nan)
        rho = np.full(len(options_data), np.nan)
